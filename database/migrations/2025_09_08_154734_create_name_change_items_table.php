...
return new class extends Migration
{
    ...
    {
        Schema::create('name_change_items', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('name_change_id')->index()->comment('名义变更单ID');
            $table->date('name_change_date')->nullable()->comment('名义变更日期');
            $table->string('name_change_status')->nullable()->comment('名义变更状态');
            $table->unsignedBigInteger('inbound_item_id')->comment('入库商品ID');
            ...
            $table->integer('quantity')->comment('名义变更数量');
    ...
    {
        Schema::dropIfExists('name_change_items');
    }
};
